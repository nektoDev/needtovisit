...
<div class="fieldcontain ${hasErrors(bean: userPlaceRelationInstance, field: 'place', 'error')} required">
    ...
    <g:select id="place" name="place.id" from="${ru.nektodev.needtovisit.Place.list()}" optionKey="id" required=""
              value="${userPlaceRelationInstance?.place?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: userPlaceRelationInstance, field: 'user', 'error')} required">
    ...
    <g:select id="user" name="user.id" from="${ru.nektodev.needtovisit.Users.list()}" optionKey="id" required=""
              value="${userPlaceRelationInstance?.user?.id}" class="many-to-one"/>
</div>
